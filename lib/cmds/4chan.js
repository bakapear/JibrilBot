...
module.exports = {
    ...
    command: async function (msg, cmd, args) {
        ...
        if (data == -1) { msg.channel.send("Invalid board!"); return; }
        if (data == -2) { msg.channel.send("Nothing found?!"); return; }
...
async function getRandom4ChanPost(board, msg) {
    ...
    if (!posts) return -2;
    let mod = msg.content.startsWith(".") ? Math.floor(Math.random() * posts.length) : 0;
    ...
    mod = msg.content.startsWith(".") ? Math.floor(Math.random() * data.length) : 0;
    return {
