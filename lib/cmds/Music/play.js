...
module.exports = {
  name: ['play', 'fplay', 'p', 'fp', 'playshuffle', 'ps'],
  ...
  command: async function (msg, cmd, args) {
    ...
    if (voice[msg.guild.id].queue && voice[msg.guild.id].queue.length) {
      let mod = msg.content.startsWith(cfg.prefix.random) ? -1 : 0
      let songs = await getYoutubeVideo(msg, args.join(' '), mod)
      if (!songs) {
...
async function getYoutubeVideo (msg, query, start) {
  ...
  if (!query.q && !query.v && query.list) {
    ...
    let ids = []
    do {
      let url = `https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&pageToken=${next}&maxResults=50&playlistId=${query.list}&key=${apiGoogle}`
      let body = (await got(url, { json: true })).body
      if (!body.items.length) return
      for (let i = 0; i < body.items.length; i++) {
        let thumb = 'https://i.imgur.com/WBB1NVX.jpg'
    ...
    if (ids.length >= 1000) {
      msg.channel.send("Oh my, that's a huge playlist! I'll see what I can do.")
    } else if (ids.length >= 500) {
      msg.channel.send("So many videos in there! Don't worry, I got this.")
    } else if (ids.length >= 250) {
      msg.channel.send('Wew, a big playlist you got there! Leave it to me.')
    }
    let times = await getDurations(ids)
    for (let song of songs) {
      song.duration = times[song.id]
    }
    return songs
  } else {
    let time = query.t
    ...
    let body = (await got(url, {
      query: { part: 'snippet', type: 'video', maxResults: '50', q: query, key: apiGoogle },
      json: true
    })).body
    ...
    return {
      ...
      img: thumb,
      duration: (await getDurations([body.items[mod].id.videoId]))[body.items[mod].id.videoId],
      time: time
    }
  }
}
  ...
  do {
    let body = (await got(url, {
      query: {
        ...
        part: 'contentDetails',
        key: apiGoogle,
        fields: 'items/contentDetails/duration,items/id'
      },
      json: true
    })).body
    for (let i = 0; i < body.items.length; i++) {
      if (!body.items[i].contentDetails) continue
      times[body.items[i].id] = formatYTTime(body.items[i].contentDetails.duration)
    }
  } while (ids.length)
  return times
}
