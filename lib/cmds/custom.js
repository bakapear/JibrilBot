...
module.exports = {
    ...
    command: async function (msg, cmd, args) {
        if (!args[0]) {
            let body = await getRandomFile(msg.author.id);
            if (!body) { msg.channel.send("You currently have no folders with content!"); return; }
            if (body.data.startsWith("http://") || body.data.startsWith("https://")) {
                if (body.data.endsWith(".ogg") || body.data.endsWith(".wav") || body.data.endsWith(".mp3") || body.data.endsWith(".mp4")) {
            ...
            msg.msg.channel.send(body.data);
            return;
        }
        if (args[0] == "list") {
        ...
        if (!args[1] || !isNaN(args[1])) {
            ...
            if (body == -3) { msg.channel.send("You currently have no folders with content!"); return; }
        ...
        if (args[1] == "rename") {
            if (!args[2]) { msg.channel.send("Please give a new name for the folder"); return; }
            let body = await renameFolder(msg.author.id, args[0], args[2]);
            if (!body) { msg.channel.send("Folder does not exist!"); return; }
            msg.channel.send("Renamed " + args[0] + " to " + body + "!");
            return;
        }
        if (args[1] == "add") {
...
async function getRandomFile(user) {
    let body = await fetchDatabase();
    if (!body || !body.hasOwnProperty(user)) return false;
    let output = [];
    for (let i = 0; i < Object.keys(body[user]).length; i++) {
        let folder = Object.keys(body[user])[i];
        output.push(...body[user][folder])
    }
    let mod = Math.floor(Math.random() * output.length);
    return { "data": output[mod] };
}