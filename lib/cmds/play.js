...
module.exports = {
    ...
    command: async function (msg, cmd, args) {
        ...
        if ("q" in data || "v" in data) {
            ...
            let high = "https://puu.sh/BdcCE.jpg";
            let med = "https://puu.sh/BdcBr.jpg";
            if (body.items[0].snippet.thumbnails) {
                if (body.items[0].snippet.thumbnails.high) high = body.items[0].snippet.thumbnails.high.url;
                if (body.items[0].snippet.thumbnails.medium) med = body.items[0].snippet.thumbnails.medium.url;
                songs.push([id, body.items[0].snippet.title, high, med]);
            }
            else {
            ...
            do {
                ...
                for (let i = 0; i < body.items.length; i++) {
                    let high = "https://puu.sh/BdcCE.jpg";
                    let med = "https://puu.sh/BdcBr.jpg";
                    if (body.items[i].snippet.thumbnails) {
                        if (body.items[i].snippet.thumbnails.high) high = body.items[i].snippet.thumbnails.high.url;
                        if (body.items[i].snippet.thumbnails.medium) med = body.items[i].snippet.thumbnails.medium.url;
                        songs.push([body.items[i].snippet.resourceId.videoId, body.items[i].snippet.title, high, med]);
                    }
                }
...
function playQueue(msg, connection, streamurl, data) {
    require("./nowplaying").command(msg, "new");
