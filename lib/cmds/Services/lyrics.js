...
module.exports = {
    ...
    command: async function (msg, cmd, args) {
        let url = `http://api.musixmatch.com/ws/1.1/track.search?q_track_artist=${encodeURIComponent(msg.content.slice(cmd.length + 1).trim())}&page_size=3&page=1&s_track_rating=desc&apikey=${api_musix}`
        ...
        url = `http://api.musixmatch.com/ws/1.1/track.lyrics.get?track_id=${body.message.body.track_list[0].track.track_id}&apikey=${api_musix}`
