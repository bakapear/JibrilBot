...
module.exports = {
    ...
    command: async function (msg, cmd, args) {
        ...
        let body = (await got(`https://api.github.com/search/code?q=${encodeURIComponent(msg.content.slice(cmd.length + 1).trim())}+in:path+extension:ogg+path:sound/chatsounds/autoadd+repo:Metastruct/garrysmod-chatsounds&access_token=${api_github}`, { json: true, headers: { "User-Agent": "Jibril" } })).body;
...
function playSound(msg, connection, streamurl, name) {
    let link = `https://raw.githubusercontent.com/Metastruct/garrysmod-chatsounds/master/${encodeURIComponent(streamurl.trim())}`;
    printSound(msg, name, link);
    dispatcher = connection.playArbitraryInput(link);
    dispatcher.setBitrate(96000);
    dispatcher.on("start", () => {
        vc.conn.player.streamingData.pausedTime = 0;
    });
    dispatcher.on("end", () => {
        dispatcher = null;
