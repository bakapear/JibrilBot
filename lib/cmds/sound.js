...
async function getSound(query, rnd) {
    let body = (await got(`https://api.github.com/search/code?q=${encodeURIComponent(query.trim())}+in:path+extension:ogg+path:sound/chatsounds/autoadd+repo:Metastruct/garrysmod-chatsounds&access_token=${api_github}`, { json: true, headers: { "User-Agent": "Jibril" } })).body
