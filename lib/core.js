...
bot.on("message", async msg => {
	...
	if (msg.content.startsWith(`<@${bot.user.id}>`) && !msg.author.bot) {
		await replika(msg)
		return
	}
...
async function replika(msg) {
	let text = msg.content.substr(`<@${bot.user.id}>`.length).trim()
	if (!text) {
		let url = "https://my.replika.ai/api/mobile/0.8/bots/" + process.env.REPLIKA_BOT
		let body = (await got(url, {
			headers: {
				"x-auth-token": process.env.REPLIKA_KEY,
				"x-user-id": process.env.REPLIKA_USER,
				"x-device-id": process.env.REPLIKA_DEVICE
			},
			json: true
		})).body
		msg.channel.send({
	...
	let body = {
		...
		payload: {
			...
			meta: {
				bot_id: process.env.REPLIKA_BOT,
				client_token: guid,
				chat_id: process.env.REPLIKA_CHAT,
				timestamp: new Date().toISOString()
			}
		},
		token: process.env.REPLIKA_TOKEN,
		auth: {
			user_id: process.env.REPLIKA_USER,
			device_id: process.env.REPLIKA_DEVICE,
			auth_token: process.env.REPLIKA_KEY
		}
	}
	let heart
