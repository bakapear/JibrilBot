...
global.playQueue = async msg => {
  if (voice[msg.guild.id].queue[0].type === 'yt') {
    let stream = ytdl(voice[msg.guild.id].queue[0].id, { audioonly: true })
    stream.on('error', e => {
      console.log('StreamError: ' + e)
    })
    voice[msg.guild.id].disp = voice[msg.guild.id].conn.playStream(stream)
...
global.showPlayMessage = (msg, title, desc, url, img, dur) => {
...
global.doChecks = (checks, stuff, val) => {
  if (!val) return []
  let arr = []
...
let { bot, voice, cfg, playQueue, showPlayMessage, addEndEvent } = global
// #endregion
...
bot.on('ready', async () => {
  ...
  if (process.env.DEBUG_PREFIXES) {
    let prefixes = process.env.DEBUG_PREFIXES.split(',')
    cfg.prefix.random = prefixes[0]
    cfg.prefix.hidden = prefixes[1]
    cfg.prefix.first = prefixes[2]
  }
...
bot.on('message', async msg => {
  ...
  if (msg.content.startsWith('https://discordapp.com/channels')) {
    let parts = msg.content.substr(msg.content.indexOf('/channels/') + 10).split('/')
    if (parts.length === 3) {
...
async function boopChat (msg) {
  if (msg.content === 'dragon') {
    msg.channel.send({
      embed: {
        title: 'Black Clovah',
        url: 'https://www.livechart.me/anime/2478'
      }
    })
  }
  if (msg.embeds[0]) {
    let alts = await boopChartAlts(msg.embeds[0].url)
    if (!alts) return
    ...
    for (let user in store) {
      ...
      let id = parseInt(alts[store[user].type].split('/').pop())
      if (store[user].anime.includes(id)) {
        users.push(`<@${user}>`)
      }
    }
    if (users.length) {
...
async function boopChartAlts (url) {
  try {
    let { body } = await got(url)
    let $ = cheerio.load(body)
    let buttons = $('a.button.expanded')
    let ani = null
    let mal = null
    for (let i = 0; i < buttons.length; i++) {
      let link = buttons[i].attribs.href
      if (link.indexOf('anilist.co') >= 0) ani = link
      if (link.indexOf('myanimelist.net') >= 0) mal = link
    }
    if (ani === null && mal === null) return null
    return { anilist: ani, mallist: mal }
