...
module.exports = {
  ...
  command: async function (msg, cmd, args) {
    try {
      let items = await getItems(args.join(' ').trim(), msg.content.startsWith(cfg.prefix.first))
      if (items === -1) {
      ...
      let mod = msg.content.startsWith(cfg.prefix.random) ? Math.floor(Math.random() * items.matches.length) : 0
